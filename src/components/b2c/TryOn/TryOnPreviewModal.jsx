  ...
  const backgroundOptions = [
    {
      ...
      name: "Hallway",
      image:
        "https://res.cloudinary.com/doiezptnn/image/upload/v1763193815/Rectangle_13_odczru.png",
    },
...
const performTryOn = async () => {
  ...
  try {
    ...
    // const response = await fetch("http://localhost:3002/api/test-tryon", {
    const response = await fetch(`https://dvyb-web-ten.vercel.app/api/test-tryon`, {
      method: "POST",
      body: formData,
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(`Gemini server error: ${err}`);
    }

    const data = await response.json();

    if (data.success && data.result) {
      const resultUrl = data.result; // already data:image/png;base64,...
      setTryOnResult(resultUrl);

      // Auto-remove background after successful try-on
      ...
      // Auto-trigger 3D video if in 3D mode
      if (is3DMode && resultUrl) {
        setTimeout(() => generateVideo(resultUrl), 1500);
      }
    } else {
      throw new Error(data.error || "No result from Gemini");
    }
  } catch (error) {
    console.error("Gemini Try-On failed:", error);
    setErrorMsg("AI try-on failed. Trying with high-quality Gemini model...");
  } finally {
  ...
  const removeBackgroundFromResult = async (imageUrl) => {
    ...
    try {
      ...
      const removeBgResponse = await fetch("https://api.remove.bg/v1.0/removebg", {
        method: "POST",
        headers: {
          "X-Api-Key": "oPpLXL42exAnFuQew7KzASML",
        },
        body: formData,
      });
