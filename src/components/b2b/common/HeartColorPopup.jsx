...
const HeartColorsPopup = ({ product, onClose, editingItem = null, userRole, onConfirm }) => {
  ...
  const handleContinue = async () => {
    ...
    if (totalQuantity < 6) {
      alert("Please add at least six items before continuing");
      return;
    }
    ...
    try {
      ...
      console.log("🟡 Calling wishlistService.addToWishlist with:", {
        productId,
      ...
      const result = await wishlistService.addToWishlist(productId, product, variants, userRole);

      console.log("✅ wishlistService.addToWishlist completed successfully:", result);
      ...
      console.error("❌ CATCH BLOCK TRIGGERED in handleContinue");
  ...
  //   const handleContinue = async () => {
  //     console.log("The items length", addedItems);
  //     const totalQuantity = addedItems.reduce((sum, item) => sum + item.quantity, 0);
  ...
  //       await wishlistService.addToWishlist(productId, product, variants, userRole);
  ...
  //       }
  //     } catch (err) {
  //       console.error("❌ [AvailColorsPopup] Error:", err);
  //       alert("Something went wrong! 1234");
  //     } finally {
  //       setLoading(false);
  //       onClose();
  //     }
...
export default HeartColorsPopup;
