...
const AvailColorsPopup = ({ product, onClose, editingItem = null, userRole, onConfirm }) => {
  ...
  const getAvailableSizesForColor = (color) => {
    ...
    if (colorKey && units[colorKey]) {
      // Get sizes available for this specific color
      return Object.keys(units[colorKey]).filter(
        (size) => !size.includes("_") && !size.includes("#")
      );
    }

    // Fallback to general sizes if color-specific not found
  ...
  const handleContinue = async () => {
    ...
    setLoading(true);
    try {
      console.log("🔍 [AvailColorsPopup] Added items:", addedItems);
...
export default AvailColorsPopup;
